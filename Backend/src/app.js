...
const SA_PRODUCTS = {
  "vendor-mama-thandi": [
    { id: "prod-vetkoek-mince", vendorId: "vendor-mama-thandi", name: "Mince Vetkoek (2-pack)", description: "Two golden vetkoek filled with spiced savoury mince.", price: 28, stock: 45, image: "/images/vetkoek-filled.jpg" },
    { id: "prod-vetkoek-atchar", vendorId: "vendor-mama-thandi", name: "Atchar Vetkoek", description: "Hot vetkoek with tangy mango atchar – a vegetarian township favourite.", price: 22, stock: 30, image: "/images/amagwinya-fried-bread.jpg" },
    { id: "prod-vetkoek-cheese", vendorId: "vendor-mama-thandi", name: "Cheese & Tomato Vetkoek", description: "Soft amagwinya with melted cheese and fresh tomato.", price: 20, stock: 40, image: "/images/vetkoek-filled.jpg" },
    { id: "prod-vetkoek-chicken", vendorId: "vendor-mama-thandi", name: "Chicken Curry Vetkoek", description: "Creamy Cape Malay-style chicken curry stuffed inside fresh vetkoek.", price: 35, stock: 25, image: "/images/amagwinya-fried-bread.jpg" },
  ],
  "vendor-bra-zakes": [
  ...
  "vendor-pap-shack": [
    { id: "prod-pap-wors", vendorId: "vendor-pap-shack", name: "Pap & Wors Plate", description: "Creamy soft pap with grilled boerewors, tomato gravy, and chakalaka.", price: 55, stock: 30, image: "/images/pap-meat-and-vegetables.jpg" },
    { id: "prod-pap-morogo", vendorId: "vendor-pap-shack", name: "Pap, Morogo & Chicken", description: "Maize pap with steamed morogo and grilled chicken pieces.", price: 65, stock: 20, image: "/images/pap-meat-and-vegetables.jpg" },
    { id: "prod-pap-umleqwa", vendorId: "vendor-pap-shack", name: "Umleqwa Plate", description: "Free-range chicken slow-cooked in a rich tomato and onion sauce, served with pap.", price: 80, stock: 15, image: "/images/pap-meat-and-vegetables.jpg" },
  ],
  "vendor-kota-king": [
    { id: "prod-kota-classic", vendorId: "vendor-kota-king", name: "Classic Kota", description: "Quarter loaf loaded with polony, fried egg, chips, atchar, and tomato sauce.", price: 35, stock: 50, image: "/images/loaded-kota.jpg" },
    { id: "prod-kota-russian", vendorId: "vendor-kota-king", name: "Russian Kota", description: "Quarter loaf with sliced Russian sausage, chips, atchar, and extra cheese.", price: 40, stock: 45, image: "/images/kota-cheese-sausage.jpg" },
    { id: "prod-kota-special", vendorId: "vendor-kota-king", name: "Kota Special (Everything!)", description: "Full load: polony, Russian, egg, atchar, chips, cheese, chakalaka, and peri-peri.", price: 55, stock: 30, image: "/images/kota-cheese-sausage-alternate.jpg" },
  ],
};
...
app.post("/api/auth/register", async (req, res) => {
  try {
    ...
    const user = { id: `user-${Date.now()}`, email, password: hashed, name, userType, phone: phone || null, vendorId };
    users.push(user);

    const token = signToken({ id: user.id, email: user.email, userType: user.userType, vendorId: user.vendorId });
...
function requireStripe(res) {
  if (!stripe) {
    res.status(503).json({ error: "Stripe is not configured. Set STRIPE_SECRET_KEY to enable card payments." });
    return false;
  }
  return true;
}
...
function requireVendor(req, res, next) {
  if (req.user.userType !== "vendor") {
    return res.status(403).json({ error: "This Stripe catalogue action is restricted to vendors." });
  }
  return next();
}
...
app.post("/api/payments/customers", verifyToken, async (req, res) => {
  ...
  try {
    ...
    return res.status(502).json({ error: error.message || "Unable to create the Stripe customer." });
  }
});
...
app.get("/api/payments/customers", verifyToken, async (req, res) => {
  ...
  try {
    ...
    return res.status(502).json({ error: error.message || "Unable to retrieve the Stripe customer." });
  }
});
...
app.post("/api/payments/create-intent", verifyToken, async (req, res) => {
  ...
  if (!order) {
    return res.status(400).json({ error: "A valid cart with product quantities is required." });
  }
  ...
  try {
    ...
    return res.status(201).json({ clientSecret: paymentIntent.client_secret, paymentIntentId: paymentIntent.id, totalAmount: order.total });
  } catch (error) {
    return res.status(502).json({ error: error.message || "Unable to initialize the Stripe payment." });
  }
});

app.get("/api/payments/intents/:paymentIntentId", verifyToken, async (req, res) => {
  if (!requireStripe(res)) return;
  try {
    const paymentIntent = await stripe.paymentIntents.retrieve(req.params.paymentIntentId);
    if (paymentIntent.metadata.customerId !== req.user.id) {
      return res.status(403).json({ error: "You cannot access this payment." });
    }
    return res.json(paymentIntent);
  } catch (error) {
    return res.status(404).json({ error: error.message || "Payment intent not found." });
  }
});
...
app.post("/api/payments/checkout-sessions", verifyToken, async (req, res) => {
  if (!requireStripe(res)) return;
  const order = calculateOrder(req.body.items);
  if (!order) return res.status(400).json({ error: "A valid cart with product quantities is required." });
  ...
  try {
    ...
    return res.status(502).json({ error: error.message || "Unable to create Stripe Checkout session." });
  }
});

app.post("/api/payments/products", verifyToken, requireVendor, async (req, res) => {
  if (!requireStripe(res)) return;
  const { name, description } = req.body;
  if (!name) return res.status(400).json({ error: "A product name is required." });
  try {
    return res.status(201).json(await stripe.products.create({ name, description, metadata: { vendorId: req.user.vendorId || "" } }));
  } catch (error) {
    return res.status(502).json({ error: error.message || "Unable to create Stripe product." });
  }
});

app.post("/api/payments/prices", verifyToken, requireVendor, async (req, res) => {
  if (!requireStripe(res)) return;
  const { product, unitAmount, recurring } = req.body;
  if (!product || !Number.isInteger(unitAmount) || unitAmount < 1) {
    return res.status(400).json({ error: "product and an integer unitAmount in cents are required." });
  }
  try {
    return res.status(201).json(await stripe.prices.create({
      product,
      currency: "zar",
    ...
    return res.status(502).json({ error: error.message || "Unable to create Stripe price." });
  }
});

app.post("/api/payments/subscriptions", verifyToken, async (req, res) => {
  if (!requireStripe(res)) return;
  if (!req.body.price) return res.status(400).json({ error: "A Stripe price ID is required." });
  try {
    ...
    return res.status(502).json({ error: error.message || "Unable to create Stripe subscription." });
  }
});
...
app.post("/api/payments/refunds", verifyToken, async (req, res) => {
  ...
  const ownedOrder = orders.find((order) => order.customerId === req.user.id && order.paymentIntentId === paymentIntentId);
  if (!paymentIntentId || !ownedOrder) return res.status(403).json({ error: "You cannot refund this payment." });
  try {
    return res.status(201).json(await stripe.refunds.create({ payment_intent: paymentIntentId }));
  } catch (error) {
    return res.status(502).json({ error: error.message || "Unable to create Stripe refund." });
  }
});

app.get("/api/orders", verifyToken, (req, res) => {
  const userOrders = orders.filter((o) => o.customerId === req.user.id);
  return res.json({ data: userOrders, meta: { total: userOrders.length, page: 1, pageSize: userOrders.length, totalPages: 1 } });
});

async function createOrder(req, res) {
  const { items, deliveryAddress, paymentMethod, notes, paymentIntentId } = req.body;
  const calculatedOrder = calculateOrder(items);
  if (!calculatedOrder) return res.status(400).json({ error: "A valid cart with product quantities is required." });

  if (paymentMethod === "Stripe") {
    if (!stripe || !paymentIntentId) {
      return res.status(400).json({ error: "A confirmed Stripe payment is required." });
    }
    ...
    try {
      ...
      if (
        ...
        return res.status(400).json({ error: "The Stripe payment does not match this order." });
      }
    } catch (error) {
      return res.status(400).json({ error: error.message || "Unable to verify the Stripe payment." });
    }
  }
...
app.get("/api/orders/vendor", verifyToken, requireVendor, (req, res) => {
  const vendorOrders = orders.filter((order) => order.vendorId === req.user.vendorId);
  return res.json({ data: vendorOrders, meta: { total: vendorOrders.length, page: 1, pageSize: vendorOrders.length, totalPages: 1 } });
});

app.get("/api/orders/:id", verifyToken, (req, res) => {
  const order = orders.find((candidate) => candidate.id === req.params.id);
  if (!order) return res.status(404).json({ error: "Order not found" });
  if (order.customerId !== req.user.id && req.user.vendorId !== order.vendorId) {
...
app.patch("/api/orders/:id/status", verifyToken, requireVendor, (req, res) => {
  ...
  if (!order) return res.status(404).json({ error: "Order not found" });
  if (req.user.vendorId !== order.vendorId) return res.status(403).json({ error: "You can only update your own orders." });
  if (!validStatuses.includes(req.body.status)) return res.status(400).json({ error: "A valid order status is required." });
...
app.post("/api/orders/:id/resend-receipt", verifyToken, async (req, res) => {
  const order = orders.find((candidate) => candidate.id === req.params.id);
  if (!order) return res.status(404).json({ error: "Order not found" });
  if (order.customerId !== req.user.id) return res.status(403).json({ error: "You can only resend your own receipt." });
  const customer = getCurrentUser(req);

  try {
    order.receipt = await sendOrderReceipt(order, customer.email);
    if (order.receipt.status !== "sent") {
      return res.status(503).json({ error: "Email receipts are not configured yet." });
    }
...
app.get("/api/orders/:id/tracking", verifyToken, (req, res) => {
  ...
  if (!order) return res.status(404).json({ error: "Order not found" });
  if (order.customerId !== req.user.id && req.user.vendorId !== order.vendorId) {
    return res.status(403).json({ error: "You cannot view this order's tracking data." });
  }
  return res.json({ orderId: order.id, status: order.status, tracking: order.tracking, updatedAt: order.updatedAt });
});

app.patch("/api/orders/:id/tracking", verifyToken, requireVendor, (req, res) => {
  const order = orders.find((candidate) => candidate.id === req.params.id);
  if (!order) return res.status(404).json({ error: "Order not found" });
  if (req.user.vendorId !== order.vendorId) {
    return res.status(403).json({ error: "You can only update tracking for your own orders." });
  }

  const { latitude, longitude } = req.body;
  if (!Number.isFinite(latitude) || !Number.isFinite(longitude) || latitude < -90 || latitude > 90 || longitude < -180 || longitude > 180) {
    return res.status(400).json({ error: "Valid latitude and longitude values are required." });
  }
  ...
  order.updatedAt = new Date().toISOString();
  return res.json({ orderId: order.id, tracking: order.tracking, updatedAt: order.updatedAt });
});
...
function getVendorDashboardStats(vendorId) {
  ...
  return {
    ...
    averageRating: SA_VENDORS.find((vendor) => vendor.id === vendorId)?.rating || 0,
    completedOrders: vendorOrders.filter((order) => order.status === "completed").length,
    pendingOrders: vendorOrders.filter((order) => order.status === "pending").length,
    returningCustomers: new Set(vendorOrders.map((order) => order.customerId)).size,
    topProducts: (SA_PRODUCTS[vendorId] || []).slice(0, 3).map((product) => ({ label: product.name, value: product.stock || 0 })),
    salesTrend: [],
...
function requireVendorId(req, res, next) {
  const vendorId = req.user.vendorId || (req.user.userType === "vendor" ? "vendor-mama-thandi" : null);
  if (!vendorId) return res.status(403).json({ error: "A vendor account is required." });
...
app.get("/api/dashboard/analytics", verifyToken, requireVendorId, (req, res) => {
  return res.json(getVendorDashboardStats(req.vendorId));
});

app.get("/api/dashboard/profile", verifyToken, (req, res) => {
  const user = getCurrentUser(req);
  const vendor = SA_VENDORS.find((candidate) => candidate.id === (req.user.vendorId || "vendor-mama-thandi"));
  return res.json({
    id: user.id,
    email: user.email,
    phone: user.phone || "",
