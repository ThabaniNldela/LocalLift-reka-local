...
export const authApi = {
  getProfile: (token: string) =>
    apiRequest<SessionUser>(`${API_CONFIG.endpoints.auth}/me`, { token }),
...
export const vendorsApi = {
  ...
  detail: (vendorId: string) =>
    apiRequest<Vendor>(`${API_CONFIG.endpoints.vendors}/${vendorId}`),

  list: (filters: VendorFilters & PaginatedParams = {}) =>
    apiRequest<ApiListResponse<VendorSummary>>(
      API_CONFIG.endpoints.vendors,
      {},
      filters,
    ),

  reviews: (vendorId: string, page = 1) =>
    apiRequest<ApiListResponse<Review>>(
      `${API_CONFIG.endpoints.vendors}/${vendorId}/reviews`,
      {},
      { page },
    ),
}
...
export const productsApi = {
  create: (token: string, payload: Partial<Product>) =>
    apiRequest<Product>(API_CONFIG.endpoints.products, {
      method: "POST",
      body: payload,
      token,
    }),
  ...
  listMine: (token: string, params: PaginatedParams = {}) =>
    apiRequest<ApiListResponse<Product>>(
      `${API_CONFIG.endpoints.products}/mine`,
      { token },
      params,
    ),
  ...
  update: (token: string, productId: string, payload: Partial<Product>) =>
    apiRequest<Product>(`${API_CONFIG.endpoints.products}/${productId}`, {
      method: "PUT",
      body: payload,
      token,
    }),
...
export const ordersApi = {
  checkout: (token: string, payload: CheckoutPayload) =>
    apiRequest<Order>(`${API_CONFIG.endpoints.orders}/checkout`, {
      method: "POST",
      body: payload,
      token,
    }),
  ...
  listMine: (token: string, params: PaginatedParams = {}) =>
    apiRequest<ApiListResponse<Order>>(
      `${API_CONFIG.endpoints.orders}/mine`,
      { token },
      params,
    ),
  ...
  listVendor: (token: string, params: PaginatedParams = {}) =>
    apiRequest<ApiListResponse<Order>>(
      `${API_CONFIG.endpoints.orders}/vendor`,
      { token },
      params,
    ),
  ...
  tracking: (token: string, orderId: string) =>
    apiRequest<{
      orderId: string
      status: OrderStatus
      tracking: NonNullable<Order["tracking"]>
      updatedAt: string
    }>(
...
export const dashboardApi = {
  analytics: (token: string) =>
    apiRequest<DashboardStats>(`${API_CONFIG.endpoints.dashboard}/analytics`, {
      token,
    }),

  profile: (token: string) =>
    apiRequest<UserProfile>(`${API_CONFIG.endpoints.dashboard}/profile`, {
      token,
    }),

  stats: (token: string) =>
    apiRequest<DashboardStats>(`${API_CONFIG.endpoints.dashboard}/stats`, {
      token,
    }),
...
export const farmersApi = {
  impact: () =>
    apiRequest<FarmerImpact>(`${API_CONFIG.endpoints.farmers}/impact`),

  listHarvests: () =>
    apiRequest<ApiListResponse<HarvestListing>>(
      `${API_CONFIG.endpoints.farmers}/harvests`,
    ),

  reserveHarvest: (token: string, harvestId: string, quantity: number) =>
    apiRequest<HarvestReservation>(
      `${API_CONFIG.endpoints.farmers}/harvests/${harvestId}/reservations`,
      {
        method: "POST",
