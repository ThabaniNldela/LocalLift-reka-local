...
export default function RegisterPage({
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    if (
      (form.userType === "vendor" || form.userType === "farmer") &&
      (!form.businessName || !form.category)
    ) {
      setLocalError(
        "Vendor and farmer accounts require a business name and category.",
      )
  ...
  return (
    <div className="mx-auto max-w-3xl space-y-6">
      <SectionHeading
        ...
        title="Join Reka Local"
        description="Register as a customer, vendor, or farmer and start trading with your community."
      />
      <Card>
        <form className="space-y-4" onSubmit={handleSubmit}>
          ...
          <div className="grid gap-4 md:grid-cols-2">
            ...
            {form.userType !== "customer" ? (
              <Input
                label={
                  form.userType === "farmer" ? "Farm name" : "Business name"
                }
                onChange={(event) =>
                  setForm((value) => ({
                    ...value,
