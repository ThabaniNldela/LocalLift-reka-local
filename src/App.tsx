...
import type { CheckoutPayload, Order, UserType } from "@/types"
...
export default function App() {
  ...
  const customerHome = useMemo(
    () => (user?.userType === "vendor" ? "vendor-dashboard" : "home"),
    [user?.userType],
  )
  ...
  const handleCheckout = async (payload: CheckoutPayload) => {
    ...
    try {
      const createdOrder = token
        ? await ordersApi.checkout(token, payload)
        : {
            id: `order-${Date.now()}`,
  ...
  if (authLoading) {
    return (
      <div className="mx-auto max-w-3xl px-4 py-10">
        <LoadingState label="Preparing Reka Local..." />
      </div>
    )
  }
  ...
  const renderPage = () => {
    switch (route.page) {
      case "home":
        return (
          <HomePage
            onBrowse={() => guardedNavigate("discover")}
            onOpenVendor={(vendorId) =>
              guardedNavigate("vendor-details", { vendorId })
            }
            onVendorJoin={() => navigate("register", { userType: "vendor" })}
          />
        )
      ...
      case "impact":
        return (
          <ImpactHubPage
            onBrowse={() => guardedNavigate("discover")}
            onVendorDashboard={() => guardedNavigate("vendor-dashboard")}
          />
        )
      ...
      case "vendor-details":
        return (
          <VendorDetailsPage
            onBack={() => guardedNavigate("discover")}
            vendorId={route.params?.vendorId || "vendor-mama-thandi"}
          />
        )
      ...
      case "cart":
        return (
          <CartPage
            onCheckout={() => guardedNavigate("checkout")}
            onContinueShopping={() => guardedNavigate("discover")}
          />
        )
      ...
      case "checkout":
        return (
          <div className="space-y-4">
            ...
            <CheckoutPage onSubmit={handleCheckout} />
          </div>
        )
      ...
      case "order-confirmation":
        return latestOrder ? (
          <OrderConfirmationPage
            onTrackOrder={() => guardedNavigate("orders")}
            order={latestOrder}
          />
        ) : (
          <NotFoundPage onReturnHome={() => guardedNavigate(customerHome)} />
        )
      ...
      case "register":
        return (
          <RegisterPage
            defaultUserType={route.params?.userType as UserType || "customer"}
            onSuccess={() => guardedNavigate(customerHome)}
          />
        )
    ...
    <>
        ...
        {!health.isConnected ? (
          <div className="mb-6">
            <Alert variant="warning">
              Backend unavailable right now. Mock data is powering the
              experience while connectivity is restored.
            </Alert>
          </div>
        ) : null}
        {renderPage()}
      ...
      {user?.userType !== "vendor" ? (
        <CartDrawer
          onCheckout={() => {
            setCartOpen(false)
            guardedNavigate("checkout")
          }}
          onClose={() => setCartOpen(false)}
          open={cartOpen && route.page !== "cart"}
        />
      ) : null}
    </>
  )
